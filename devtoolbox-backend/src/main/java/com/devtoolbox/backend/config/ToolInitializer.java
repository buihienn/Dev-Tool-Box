    ...
    public CommandLineRunner initializeTools(ToolRepository toolRepository, CategoryRepository categoryRepository) {
        return args -> {
            ...
            List<Tool> tools = List.of(
                new Tool("token", "Trình tạo mã thông báo", "Tạo mã thông báo ngẫu nhiên với độ dài tùy chỉnh và nhiều tùy chọn khác nhau.", true, false, false, categoryRepository.findById("crypto").orElse(null)),
                new Tool("hash", "Mã hóa văn bản", "Mã hóa văn bản của bạn bằng nhiều thuật toán mã hóa khác nhau như MD5, SHA-1, SHA-256 và nhiều thuật toán khác.", true, false, false, categoryRepository.findById("crypto").orElse(null)),
                new Tool("bcrypt", "Bcrypt", "Mã hóa mật khẩu bằng thuật toán BCrypt - một thuật toán an toàn và đáng tin cậy cho việc lưu trữ mật khẩu.", true, false, false, categoryRepository.findById("crypto").orElse(null)),
                new Tool("ulid", "Trình tạo ULID", "Tạo các định danh duy nhất có thứ tự theo thời gian (ULID) - thay thế cho UUID với khả năng sắp xếp theo thời gian.", true, false, false, categoryRepository.findById("crypto").orElse(null)),
                ...
                new Tool("math", "Đánh giá biểu thức toán học", "Tính toán các biểu thức toán học phức tạp với nhiều hàm và hằng số khác nhau.", true, false, true, categoryRepository.findById("math").orElse(null)),
                ...
                new Tool("percentage-calculator", "Máy tính phần trăm", "Tính toán phần trăm, tăng/giảm phần trăm và các bài toán phần trăm khác.", true, false, true, categoryRepository.findById("math").orElse(null)),
                ...
                new Tool("temperature-converter", "Bộ chuyển đổi nhiệt độ", "Chuyển đổi nhiệt độ giữa các đơn vị Celsius, Fahrenheit, Kelvin và Rankine.", true, false, true, categoryRepository.findById("measurement").orElse(null)),
                new Tool("benchmark", "Trình tạo bảng đánh giá", "Tạo và phân tích các bảng đánh giá hiệu năng với biểu đồ và thống kê.", true, false, true, categoryRepository.findById("measurement").orElse(null)),
                new Tool("lorem-ipsum", "Trình tạo văn bản Lorem Ipsum", "Tạo văn bản giả Lorem Ipsum với số lượng từ, câu, đoạn tùy chỉnh.", true, false, true, categoryRepository.findById("text").orElse(null)),
                new Tool("text-statistics", "Thống kê văn bản", "Phân tích văn bản và hiển thị các thống kê về số từ, câu, ký tự và độ đọc.", true, false, true, categoryRepository.findById("text").orElse(null)),
                ...
                new Tool("phone-parser", "Trình phân tích và định dạng số điện thoại", "Phân tích và định dạng số điện thoại theo các chuẩn quốc tế khác nhau.", true, false, true, categoryRepository.findById("data").orElse(null)),
                new Tool("iban", "Kiểm tra và phân tích số IBAN", "Kiểm tra tính hợp lệ và phân tích các thành phần của số IBAN ngân hàng.", true, false, true, categoryRepository.findById("data").orElse(null)),
                new Tool("credit-card", "Xác thực thẻ tín dụng", "Xác thực số thẻ tín dụng theo thuật toán Luhn và nhận dạng loại thẻ.", true, false, true, categoryRepository.findById("data").orElse(null)),
                new Tool("mac-address", "Trình tạo địa chỉ MAC", "Tạo địa chỉ MAC ngẫu nhiên hoặc theo nhà sản xuất cụ thể.", true, false, true, categoryRepository.findById("network").orElse(null)),
                ...
                new Tool("ipv4-range", "Mở rộng dải địa chỉ IPv4", "Mở rộng dải địa chỉ IPv4 CIDR thành danh sách các địa chỉ riêng lẻ.", true, false, true, categoryRepository.findById("network").orElse(null))
            );
